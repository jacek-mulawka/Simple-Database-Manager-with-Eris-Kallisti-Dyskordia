unit Database__Trigger_Modify;{11.Paü.2023}
...
type
  TDatabase__Trigger_Modify_Form = class( TForm )
    ...
    Trigger_Event_Etiquette_Label: TLabel;
    Trigger_Event_ComboBox: TComboBox;
  ...
  private
    ...
    function Quotation_Sign__DTM() : string;
  public
    { Public declarations }
    modified__dtm,
    sql__quotation_sign__use__dtm,
    trigger_edit__dtm
      : boolean;

    trigger_name__dtm,
    database_type__dtm,
    sql__quotation_sign__dtm
      : string;
...
const
  database__trigger_modify__event_c : string = 'TRIGGER_EVENT';
  database__trigger_modify__events_list__file_name_c : string = 'Database__Trigger_Modify__Events_List.txt';
  database__trigger_modify__sql__trigger__data__file_name_c : string = 'Database__Trigger_Modify__Trigger__Data__sql.txt';
  database__trigger_modify__sql__trigger__recreate__file_name_c : string = 'Database__Trigger_Modify__Trigger__Recreate__sql.txt';

var
  Database__Trigger_Modify_Form: TDatabase__Trigger_Modify_Form;
...
procedure TDatabase__Trigger_Modify_Form.Caret_Position_Display();
begin
...
begin
  ...
  if sql__quotation_sign__use__dtm then
    begin

      Result := sql__quotation_sign__dtm;
...
constructor TDatabase__Trigger_Modify_Form.Create( AOwner : TComponent; sdbm_f : Common.TSDBM );
begin
...
procedure TDatabase__Trigger_Modify_Form.FormCreate( Sender: TObject );
begin
...
procedure TDatabase__Trigger_Modify_Form.FormShow( Sender: TObject );
var
  zti_1,
  zti_2
    : integer;
...
begin
  ...
  if trigger_edit__dtm then
    begin

      Trigger_Name_Edit.Enabled := false;
      Trigger_Name_Edit.Text := trigger_name__dtm;
  ...
  zts_1 := Common.Text__File_Load(  Common.Databases_Type__Directory_Path__Get( database_type__dtm ) + Common.trigger_modify__code_completion_list__file_name_c  );

  if Trim( zts_1 ) = '' then
    begin

      Log_Memo.Lines.Add( Translation.translation__messages_r.file_not_found___default_value_used + ' (' + Common.Databases_Type__Directory_Path__Get( database_type__dtm ) + Common.trigger_modify__code_completion_list__file_name_c + ').' );
  ...
  zts_2 := Common.Text__File_Load(  Common.Databases_Type__Directory_Path__Get( database_type__dtm ) + Common.sql_editor__code_completion_list__file_name_c  );

  if Trim( zts_2 ) = '' then
    begin

      Log_Memo.Lines.Add( Translation.translation__messages_r.file_not_found___default_value_used + ' (' + Common.Databases_Type__Directory_Path__Get( database_type__dtm ) + Common.sql_editor__code_completion_list__file_name_c + ').' );
  ...
  sql__recreate_g := Common.Text__File_Load(  Common.Databases_Type__Directory_Path__Get( database_type__dtm ) + database__trigger_modify__sql__trigger__recreate__file_name_c  );

  if Trim( sql__recreate_g ) = '' then
    begin

      Log_Memo.Lines.Add( Translation.translation__messages_r.file_not_found___default_value_used + ' (' + Common.Databases_Type__Directory_Path__Get( database_type__dtm ) + database__trigger_modify__sql__trigger__recreate__file_name_c + ').' );

      sql__recreate_g :=
        'recreate trigger __TRIGGER_NAME__ ' +
  ...
  zts_1 := Common.Text__File_Load(  Common.Databases_Type__Directory_Path__Get( database_type__dtm ) + Common.trigger_modify__activity_list__file_name_c  );

  if Trim( zts_1 ) = '' then
    begin

      Log_Memo.Lines.Add( Translation.translation__messages_r.file_not_found___default_value_used + ' (' + Common.Databases_Type__Directory_Path__Get( database_type__dtm ) + Common.trigger_modify__activity_list__file_name_c + ').' );
  ...
  zts_1 := Common.Text__File_Load(  Common.Databases_Type__Directory_Path__Get( database_type__dtm ) + database__trigger_modify__events_list__file_name_c  );

  if Trim( zts_1 ) = '' then
    begin

      Log_Memo.Lines.Add( Translation.translation__messages_r.file_not_found___default_value_used + ' (' + Common.Databases_Type__Directory_Path__Get( database_type__dtm ) + database__trigger_modify__events_list__file_name_c + ').' );

      zts_1 :=
        'on connect' + #13 + #10 +
        'on disconnect' + #13 + #10 +
        'on transaction commit' + #13 + #10 +
        'on transaction rollback' + #13 + #10 +
        'on transaction start' + #13 + #10;

    end;

  Common.Items_From_Text_Add( Trigger_Event_ComboBox.Items, zts_1 );

  if Trigger_Event_ComboBox.Items.Count > 0 then
    Trigger_Event_ComboBox.ItemIndex := 0;
  ...
  if trigger_edit__dtm then
    begin
      ...
      Trigger_Event_ComboBox.ItemIndex := -1;


      if trigger_modify_sdbm.Query__Active() then
        trigger_modify_sdbm.Query__Close();

      zts_1 := Common.Text__File_Load(  Common.Databases_Type__Directory_Path__Get( database_type__dtm ) + database__trigger_modify__sql__trigger__data__file_name_c  );

      if Trim( zts_1 ) = '' then
        begin

          Log_Memo.Lines.Add( Translation.translation__messages_r.file_not_found___default_value_used + ' (' + Common.Databases_Type__Directory_Path__Get( database_type__dtm ) + database__trigger_modify__sql__trigger__data__file_name_c + ').' );
          ...
          zts_1 :=
            ...
            ' ' +
            'from RDB$TRIGGERS ' +
            'where RDB$TRIGGERS.RDB$RELATION_NAME is null ' +
      ...
      trigger_modify_sdbm.Query__Param_By_Name__Set( Common.name__trigger__name__small_letters_c, trigger_name__dtm, Log_Memo );
        ...
        begin
          ...
          if trigger_modify_sdbm.Query__Record_Count() <> 1 then
            begin
              ...
              Application.MessageBox( PChar(Translation.translation__messages_r.trigger_not_found + #13 + #13 + trigger_name__dtm + Translation.translation__messages_r.query_output_count_ + ' ' + trigger_modify_sdbm.Query__Record_Count().ToString() + #13 + #13 + zts_1 + '.'), PChar(Translation.translation__messages_r.error), MB_OK + MB_ICONEXCLAMATION );
      ...
      if trigger_modify_sdbm.Query__Active() then
        begin
          ...
          try
            trigger_modify_sdbm.Query__Field_By_Name( database__trigger_modify__event_c ).AsString;

            for zti_1 := 0 to Trigger_Event_ComboBox.Items.Count - 1 do
              if Pos( Trigger_Event_ComboBox.Items[ zti_1 ], trigger_modify_sdbm.Query__Field_By_Name( database__trigger_modify__event_c ).AsString ) > 0 then
                begin

                  Trigger_Event_ComboBox.ItemIndex := zti_1;
            ...
            on E : Exception do
              begin
                ...
                Application.MessageBox( PChar(Translation.translation__messages_r.failed_to_read_event_column_name + #13 + #13 + E.Message + #13 + #13 + zts_1 + '.'), PChar(Translation.translation__messages_r.error), MB_OK + MB_ICONEXCLAMATION );
...
procedure TDatabase__Trigger_Modify_Form.FormDestroy( Sender: TObject );
begin
...
procedure TDatabase__Trigger_Modify_Form.Sql_Prepare_ButtonClick( Sender: TObject );
var
  zts : string;
begin
  ...
  if Trim( Trigger_Event_ComboBox.Text ) = '' then
    begin

      Trigger_Event_ComboBox.SetFocus();
      Application.MessageBox( PChar(Translation.translation__messages_r.trigger_event_should_not_be_empty), PChar(Translation.translation__messages_r.warning), MB_OK + MB_ICONEXCLAMATION );
      Exit;
  ...
  zts := StringReplace( zts, Common.sql__word_replace_separator_c + database__trigger_modify__event_c + Common.sql__word_replace_separator_c, Trigger_Event_ComboBox.Text, [ rfReplaceAll ] );
  zts := StringReplace( zts, Common.sql__word_replace_separator_c + Common.name__trigger__name__big_letters_c + Common.sql__word_replace_separator_c, Self.Quotation_Sign__DTM() + Trigger_Name_Edit.Text + Self.Quotation_Sign__DTM(), [ rfReplaceAll ] );
...
procedure TDatabase__Trigger_Modify_Form.Execute_ButtonClick( Sender: TObject );
var
  ztb : boolean;
...
procedure TDatabase__Trigger_Modify_Form.Code_Examples_ButtonClick( Sender: TObject );
var
  zts : string;
begin
  ...
  zts := Common.Text__File_Load(  Common.Databases_Type__Directory_Path__Get( database_type__dtm ) + Common.trigger_modify__code_examples__file_name_c  );

  if Trim( zts ) = '' then
    begin

      Log_Memo.Lines.Add( Translation.translation__messages_r.file_not_found___default_value_used + ' (' + Common.Databases_Type__Directory_Path__Get( database_type__dtm ) + Common.trigger_modify__code_examples__file_name_c + ').' );
...
procedure TDatabase__Trigger_Modify_Form.Log_MemoKeyDown( Sender: TObject; var Key: Word; Shift: TShiftState );
begin
...
procedure TDatabase__Trigger_Modify_Form.Sql_MemoKeyDown( Sender: TObject; var Key: Word; Shift: TShiftState );
begin
...
procedure TDatabase__Trigger_Modify_Form.Trigger_Source_MemoClick( Sender: TObject );
begin
...
procedure TDatabase__Trigger_Modify_Form.Trigger_Source_MemoKeyDown( Sender: TObject; var Key: Word; Shift: TShiftState );
begin
...
procedure TDatabase__Trigger_Modify_Form.Trigger_Source_MemoKeyUp( Sender: TObject; var Key: Word; Shift: TShiftState );
begin
...
procedure TDatabase__Trigger_Modify_Form.Trigger_Source_SynEditEnter( Sender: TObject );
begin
...
procedure TDatabase__Trigger_Modify_Form.Trigger_Source_SynEditClick( Sender: TObject );
begin
...
procedure TDatabase__Trigger_Modify_Form.Trigger_Source_SynEditKeyUp( Sender: TObject; var Key: Word; Shift: TShiftState );
begin
...
procedure TDatabase__Trigger_Modify_Form.Trigger_Source_SynEditReplaceText( Sender: TObject; const ASearch, AReplace: string; Line, Column: Integer; var Action: TSynReplaceAction );
begin
...
procedure TDatabase__Trigger_Modify_Form.Trigger_Source__SynCompletionProposalCodeCompletion( Sender: TObject; var Value: string; Shift: TShiftState; Index: Integer; EndToken: Char );
begin
...
procedure TDatabase__Trigger_Modify_Form.Trigger_Source__SynCompletionProposalAfterCodeCompletion( Sender: TObject; const Value: string; Shift: TShiftState; Index: Integer; EndToken: Char );
begin
